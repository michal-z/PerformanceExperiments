...
void PointCloud::Draw() const
{
	...
	cmdList->SetGraphicsRootConstantBufferView(0, m_RootCbGpuAddr[frameIdx]);
...
void PointCloud::CreateBuffers()
{
	...
	{	// vertex buffer
		auto vbDesc = CD3DX12_RESOURCE_DESC::Buffer(k_PointCount * sizeof(XMFLOAT3));
		VHR(device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
		...
		m_PointVbView.StrideInBytes = sizeof(XMFLOAT2);
		m_PointVbView.SizeInBytes = 3 * m_PointVbView.StrideInBytes;

		XMFLOAT3* ptr;
		VHR(m_PointVb->Map(0, &CD3DX12_RANGE(0, 0), (void**)&ptr));

//		for (uint32_t pointIdx = 0; pointIdx < k_PointCount; ++pointIdx)
	//	{
		*ptr++ = XMFLOAT3(-0.5f, -0.5f, 0.0f);
		*ptr++ = XMFLOAT3(0.5f, -0.5f, 0.0f);
		*ptr++ = XMFLOAT3(0.0f, 0.5f, 0.0f);
	//	}
	}
